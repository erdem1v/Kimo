...
class _SolvePoolScreenState extends State<SolvePoolScreen> {
  ...
  /// Doğru şık — artık soruyla birlikte GELMİYOR, cevaptan sonra sunucudan
  /// öğreniliyor. Cevaplanana kadar null.
  int? _revealedCorrect;

  /// Cevap sunucuya gidiyor; ikinci dokunuşu engelle.
  ...
  Future<void> _load() async {
    ...
    try {
      final List<PublicQuestion> items = await poolRepository
  ...
  /// Cevabı sunucuya gönderir. Doğruluğu ve doğru şıkkı SUNUCU söyler;
  /// istemci artık cevabı önceden bilmiyor (correct_index havuz yükünden
  /// çıkarıldı — bkz. 0035 göçü).
  ///
  /// Hata yutulmuyor: ağ yoksa kullanıcı görüyor ve tekrar deneyebiliyor.
  /// Tekrar denemek güvenli — sunucu aynı soru için ikinci kez XP vermiyor
  /// (question_attempts birincil anahtarı).
  Future<void> _pick(int i) async {
    ...
    final AnswerResult res;
    try {
      res = await poolRepository.answerPoolQuestion(_current.id, i);
    } catch (_) {
      // Sunucu yanıt verdi ve reddetti (ör. soru artık havuzda değil).
      // Bunu kuyruğa almak anlamsız; kullanıcıya söyle.
      if (!mounted) return;
      setState(() {
        _selected = null;
        _submitting = false;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Cevap gönderilemedi. Tekrar dene.')),
      );
      return;
    }
    if (!mounted) return;
    ...
    if (res.queued) {
      // Ağ yoktu: cevap kuyrukta, kaybolmadı. Doğruluğu sunucu belirlediği
      // için sonucu ŞİMDİ gösteremiyoruz; bağlantı gelince toplamlar düzelir.
      // Seri "bugün soru çözdüm" demek; doğruluktan bağımsız ve çevrimdışıyken
      // de geçerli. XP eklemiyoruz: doğru mu bilmiyoruz, onu sunucu verecek.
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Bağlantı yok. Cevabın kaydedildi, sonucu bağlantı gelince '
            'göreceksin.',
          ),
        ),
      );
    ...
    // Yerel iyimser değerleri sunucunun gerçeğiyle uzlaştır.
    gameProgress.applyServerTotals(res.totals);
    setState(() {
      _revealedCorrect = res.correctIndex;
      _answered = true;
      _submitting = false;
    });
  }
  ...
  Widget _letter(PublicQuestion q, int i) {
    ...
    if (_answered) {
      if (_revealedCorrect == null) {
        // Sonuç henüz bilinmiyor (çevrimdışı, cevap kuyrukta): seçimi NÖTR
        // göster. Kırmızı, "yanlış cevapladın" demek olurdu ve bunu bilmiyoruz.
        if (i == _selected) {
          bg = AppColors.blueBg;
    ...
    return GestureDetector(
      onTap: (_answered || _submitting) ? null : () => _pick(i),
      child: Container(
        width: 54,
        height: 54,
