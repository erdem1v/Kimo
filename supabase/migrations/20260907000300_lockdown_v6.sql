-- 0073 — Kolon kilidi v6 (`mistakes.curriculum` sınıflandırıldı)
--
-- 0026/0036/0047/0053/0064/0069 ile aynı desen. NEDEN YENİ BİR GÖÇ: her
-- lockdown bloğu YALNIZCA kendi çalıştığı andaki katalogla karşılaştırma
-- yapıyor; 0071'in eklediği sütun 0069 koşarken henüz yoktu.
--
-- v5'ten tek fark: `mistakes.curriculum` → KİLİTLİ.
--
-- Neden istemciye kapalı: değeri sunucu yazıyor (`my_curriculum()` varsayılanı,
-- JWT metadata'sından). Açık olsaydı kullanıcı satırının müfredatını
-- değiştirip doğrulama tetikleyicisini BAŞKA bir ağaca yönlendirebilir ve
-- ağaç dışı bir konuyu geçerli gösterebilirdi — yani yazma doğrulamasının
-- tamamı bir sütun beyanına dayanırdı.
--
-- KAPSAM DIŞI, bilinçli: `curriculum_topics`, `curriculum_aliases` ve
-- `curriculum_meta` sınıflandırma döngüsünde DEĞİL. Üçü de `revoke all` ile
-- tamamen kapalı (0071) ve 0074'ün "yalnızca-sunucu" kapısı onları ayrıca
-- doğruluyor — `ai_result_cache` ve `signup_throttle` ile aynı muamele.
...
comment on table public.mistakes is
  ...
  'güncel liste 0073''tedir. İstemcinin DELETE yetkisi 0069''da geri alındı — '
...
comment on table public.profiles is
  'Kolon yazma izinleri lockdown göçlerinde beyaz listeyle yönetilir; en '
  'güncel liste 0073''tedir. İstemci yalnızca avatar_path yazabilir; '
