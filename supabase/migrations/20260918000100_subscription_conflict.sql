-- 0103 — Başka hesaba bağlı makbuz: 503 DEĞİL, karara bağlanmış `false` (Task 17)
--
-- BULGU (T17-8). `subscriptions_txn_uniq` bir makbuzun iki hesaba bağlanmasını
-- KATALOG düzeyinde yasaklıyor — doğru karar. Ama `apply_subscription` bu
-- ihlali hiç ele almıyordu: ikinci hesap aynı makbuzu gönderdiğinde INSERT
-- `unique_violation` (23505) fırlatıyor, hata PostgREST üzerinden edge
-- fonksiyonuna sızıyor ve `verify-purchase` onu genel `catch`inde
-- **503 "magaza dogrulamasi basarisiz"** olarak döndürüyordu.
--
-- NEDEN ÖNEMLİ: 503 "geçici arıza, yine dene" demek. İstemci satın almayı
-- tamamlamıyor, mağaza teslimi tekrarlıyor, kullanıcı aynı hatayı tekrar
-- tekrar görüyor ve hiçbir yerde NE olduğu yazmıyor. Oysa durum kalıcı ve
-- karara bağlanmış: bu makbuz başkasının.
--
-- Fonksiyonun sözleşmesi zaten "karara bağlandıysa false" — bilinmeyen makbuz
-- ve yanlış sır dalları bunu yapıyor. Çakışma dalı da aynı yere bağlanıyor;
-- `verify-purchase` o zaman kendi 409'unu döndürüyor ("defter yazilamadi
-- (sir? cakisma?)" — mesaj bugüne kadar ERİŞİLEMEZDİ) ve `store-notify` de
-- mağazaya 200 dönüp sonsuz yeniden teslimi kesiyor.
--
-- `create or replace`: dönüş tipi ve imza AYNI, yalnızca gövde değişiyor.
--
-- @UNDO
-- Gövdeyi 0092'deki hâline döndürün (aşağıdaki `begin ... exception` bloğunu
-- çıkarıp düz INSERT bırakın).
