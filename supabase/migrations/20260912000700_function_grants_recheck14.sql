-- 0085 — Fonksiyon yetkisi beyaz listesi (Task 12 · P5 kapısı)
--
-- 0082 ile aynı kapı, güncellenmiş listeyle.
--
-- LİSTEYE GİREN (Task 12 · P5):
--   refund_ai_use → okunamayan fotoğrafın hakkını geri veriyor (Tur 7 · n4:
--       "hak sayımı yalnızca okunabilen fotoğraflar için düşer"). Edge
--       fonksiyon KULLANICININ JWT'siyle çağırıyor, yani `authenticated`
--       açık olmak zorunda. Kötüye kullanım `ai_refund_daily` ile sınırlı ve
--       fonksiyon İDEMPOTENT — iki kez çağrılmak hak BASMIYOR.
--
-- `consume_ai_use` LİSTEDE KALIYOR ama 0083'te DROP + CREATE edildi (dönüş
-- tipine `call_id` eklendi). Ad aynı olduğu için bu kapı etkilenmiyor;
-- `create or replace` kullanılsaydı ikinci bir aşırı yükleme doğar ve mevcut
-- çağrılar "function is not unique" ile patlardı.
--
-- Bu dosyanın geri kalanı 0082'nin AYNISI. Kapılar kümülatif.
