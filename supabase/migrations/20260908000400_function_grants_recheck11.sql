-- 0078 — Fonksiyon yetkisi beyaz listesi (Task 10 kapısı)
--
-- 0029, 0033, 0038, 0049, 0054, 0056, 0061, 0065, 0070 ve 0074 ile aynı kapı,
...
--
-- LİSTEYE GİRENLER (Task 10):
--   ai_state        → `my_daily_state` görünümünün SELECT listesinde DOĞRUDAN
--       çağrılıyor. `security_invoker = false` görünümlerde tablo erişimi
--       görünüm sahibinin yetkisiyle denetlenir ama FONKSİYON çağrısının
--       EXECUTE'u ÇAĞIRANA karşı denetlenir — kapalı olsaydı görünüm her
--       kullanıcıda 42501 atardı (0040'ın 121-125. satırlarındaki ders).
--       Sıfır argümanlı ve `auth.uid()` okuyor, yani doğrudan çağrılsa bile
--       başkasının kotasını sızdırmıyor.
--   start_ad_reward → istemci ödüllü reklam öncesi nonce'u buradan alıyor.
--
-- LİSTEDEN ÇIKANLAR: `daily_ai_quota` ve `istanbul_day_reset` 0075'te
-- DÜŞÜRÜLDÜ. Listede kalsalardı `v_miss` kontrolü "beyaz listedeki fonksiyon
-- çağrılamıyor" diye patlar ve göç tüm `db reset`i kırardı.
--
-- LİSTEYE BİLEREK GİRMEYENLER: `config_int`, `user_tier`,
-- `istanbul_month_reset`, `prune_ai_calls`, `prune_ad_rewards`. Beşi de
-- yalnızca diğer definer fonksiyonlardan çağrılıyor ve definer gövdesinde
-- EXECUTE denetimi fonksiyon SAHİBİNE karşı yapılıyor — yani kapalı olmaları
-- çalışmalarını engellemiyor. Aşağıdaki revoke döngüsü onları kapatıyor; bu
-- İSTENEN davranış.
--
-- `grant_ad_reward` AYRI BİR DİZİDE (`v_anon_only`). Bu kapının tek şekli var
-- — "listede yoksa revoke, listedeyse `authenticated` çağırabilmeli" — ve o
-- fonksiyon YALNIZCA `anon`a açık olmak zorunda (AdMob geri çağrısında
-- kullanıcı JWT'si yok). `v_keep`e konsaydı `v_miss` patlardı; hiç
-- konmasaydı aşağıdaki döngü 0076'nın verdiği anon grant'ını sessizce geri
-- alırdı ve ödüller bir daha hiç verilmezdi. İkinci dizi o yüzden var ve
-- kendi POZİTİF iddiasını taşıyor.
...
-- authenticated`tan revoke ediyor — yani 0076'nın `grant_ad_reward`a verdiği
-- anon yetkisini de aldı. Burası onu geri veriyor ve gerçekten öyle olduğunu
-- İDDİA EDİYOR: yalnızca geri vermek, bir sonraki düzenlemede sessizce
-- düşmesine izin verirdi.
do $mig$
declare
  v_anon_only text[] := array['grant_ad_reward'];
  r    record;
  v_bad text[];
begin
  for r in
  ...
  if v_bad is not null then
    raise exception
      'grant_ad_reward authenticated tarafından ÇAĞRILABİLİYOR: % — istemci '
      'kendine reklam hakkı basabilir', v_bad;
  end if;
end
$mig$;
