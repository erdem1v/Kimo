-- 0101 — fonksiyon yetkisi kapısı v17 (Task 17)
--
-- 0095'in (recheck16) AYNISI, tek fark: `set_quiet_hours` beyaz listeye
-- giriyor. Kapılar kümülatif ve her yeni fonksiyon kendi kapısını gerektiriyor
-- — kapı `v_keep` dışındaki HER fonksiyondan EXECUTE'u geri alıyor ve
-- listedeki her adın `authenticated` tarafından çağrılabildiğini ayrıca
-- doğruluyor. Yeni bir ad eklenip kapı yazılmazsa fonksiyon "PUBLIC'e açık
-- doğar" kuralından hiç geçmemiş olur; `check_sql`in 3b kontrolü tam bunu
-- yakalıyor ve bu göç o uyarıyı kapatıyor.
--
-- set_quiet_hours → sessiz saat aralığını yazan tek yol (0100). Sütunlar
--     kilitli: `profiles`ın istemciye açık tek sütunu `avatar_path` ve 05 ile
--     31 numaralı mutasyonların geri almaları o listeyi elle yeniden kuruyor,
--     yani yeni bir sütunu açmak yetkinin sessizce kaybolması demekti.
