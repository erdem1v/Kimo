-- 0077 — Kolon kilidi v7 (`premium_until`, `ai_calls`, `ad_rewards`) — Task 10
--
-- 0026/0036/0047/0053/0064/0069/0073 ile aynı desen. NEDEN YENİ BİR GÖÇ: her
-- lockdown bloğu YALNIZCA kendi çalıştığı andaki katalogla karşılaştırma
-- yapıyor; 0075'in eklediği sütun ve tablolar 0073 koşarken henüz yoktu.
-- Eskisini güncellemek YETMEZ.
--
-- v6'dan üç fark:
--
--  1. `profiles.premium_until` → KİLİTLİ. Bloğun kendisi tablo düzeyinde
--     `revoke insert, update` yapıp yalnızca `avatar_path`'i geri verdiği için
--     sütun zaten kapalı doğuyor; listeye yazılması (b) kontrolünün
--     "sınıflandırılmamış kolon" diye patlamasını önlüyor VE niyeti katalogda
--     belgeliyor. Açık olsaydı kullanıcı kendini premium yapardı — kota
--     rejiminin tamamı bu tek sütuna dayanıyor.
--
--  2. `public.ai_calls` → tamamen kilitli. AI analiz çağrı defteri; kayan
--     pencere, aylık cap ve anonim ömür cap'inin tek kaynağı. Bir satır
--     eklenebilse kullanıcı kendi hakkını tüketmiş gösterebilir (zararsız),
--     bir satır SİLİNEBİLSE kendine sınırsız hak açardı.
--
--  3. `public.ad_rewards` → tamamen kilitli. `status` yazılabilir olsaydı
--     istemci kendi bekleyen satırını `granted` yapıp reklamı hiç izlemeden
--     hak kazanırdı; SSV tasarımının tamamı o tek sütuna dayanıyor.
--
-- KAPSAM DIŞI, bilinçli: `curriculum_*`, `ai_result_cache`, `signup_throttle`
-- sınıflandırma döngüsünde DEĞİL — hepsi `revoke all` ile tamamen kapalı ve
-- 0078'in "yalnızca-sunucu" kapısı onları ayrıca doğruluyor.
...
comment on table public.mistakes is
  ...
  'güncel liste 0077''dedir. İstemcinin DELETE yetkisi 0069''da geri alındı — '
...
comment on table public.profiles is
  'Kolon yazma izinleri lockdown göçlerinde beyaz listeyle yönetilir; en '
  'güncel liste 0077''dedir. İstemci yalnızca avatar_path yazabilir; '
