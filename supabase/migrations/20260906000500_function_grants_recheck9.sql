-- 0070 — Fonksiyon yetkisi beyaz listesi (Task 08 kapısı)
--
-- 0029, 0033, 0038, 0049, 0054, 0056, 0061 ve 0065 ile aynı kapı, güncellenmiş
...
--
-- Task 08'in eklediği çağrılabilir fonksiyonlar:
--   has_birth_year       → POLİTİKA İÇİNDEN çağrılıyor (mistakes ve
--       storage.objects INSERT). Açık olmak ZORUNDA; kapalı olsaydı politika
--       ifadesi hata verir ve iki yazma yolu birden kırılırdı (is_suspended'ın
--       aynı gerekçesi, 0052'nin dersi).
--   ai_age_ok            → analyze-question edge function'ının yaş kapısı.
--       Parametresiz: kimin sorulduğuna istemci karar veremiyor.
--   profiles_by_ids      → profiles_public görünümünün TEK istemci kapısı;
--       görünümün kendisi 0068'de kapatıldı.
--   my_blocked_users     → engellenenler ekranı (A-8); auth.uid()'e kilitli.
--
-- BİLEREK LİSTEDE OLMAYANLAR (0065'in listesine ek yok, gerekçeler orada):
--   mistakes_photo_violation, hook_before_user_created, prune_*, send_push,
--   handle_new_user / assign_friend_code / generate_friend_code (tetikleyici
--   ve iç yardımcılar — EXECUTE yetkisine ihtiyaçları yok).
