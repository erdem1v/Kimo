-- 0080 — Fonksiyon yetkisi beyaz listesi (Task 12 kapısı)
--
-- 0029, 0033, 0038, 0049, 0054, 0056, 0061, 0065, 0070, 0074 ve 0078 ile aynı
-- kapı, güncellenmiş listeyle. Postgres her YENİ fonksiyonda EXECUTE'u tekrar
-- PUBLIC'e verdiği için bu kontrol her paketin sonunda tekrarlanıyor.
--
-- LİSTEYE GİREN (Task 12 · P0):
--   feature_flags → `my_daily_state` görünümünün SELECT listesinde DOĞRUDAN
...
--       kullanıcıda 42501 atardı. `ai_state` ile birebir aynı gerekçe.
--       Sıfır argümanlı ve kullanıcıya özel hiçbir şey okumuyor.
--
-- LİSTEYE BİLEREK GİRMEYEN: `config_bool`. `config_int` ile aynı durum —
-- yalnızca `feature_flags()` içinden çağrılıyor ve definer gövdesinde EXECUTE
-- denetimi fonksiyon SAHİBİNE karşı yapılıyor, yani kapalı olması çalışmasını
-- engellemiyor. Aşağıdaki revoke döngüsü onu kapatıyor; bu İSTENEN davranış ve
-- `supabase/tests/310_feature_flags.sql` bunu ayrıca iddia ediyor. İstemciye
-- açılması, istemcinin `app_config`'teki HERHANGİ bir anahtarı yoklayabilmesi
-- demekti — orada `push_service_key` duruyor.
--
-- LİSTEDEN ÇIKAN: yok.
--
-- Bu dosyanın geri kalanı 0078'in AYNISI. Kapılar kümülatif: her recheck
-- öncekilerin iddialarını da tekrarlıyor, çünkü her biri YALNIZCA kendi
-- çalıştığı andaki katalogla karşılaştırma yapıyor.
