-- 0087 — Fonksiyon yetkisi beyaz listesi (Task 12 · P6 kapısı)
--
-- 0085 ile aynı kapı, güncellenmiş listeyle. Task 12'nin SON kapısı.
--
-- LİSTEYE GİRENLER (Task 12 · P6):
--   my_pair_streaks   → arkadaş listesi satırındaki ortak seri rozetini
--       besliyor. Parametresiz ve `auth.uid()`e kilitli.
--   start_pair_streak → çözümden sonraki "ortak seriniz başlasın" çağrısı.
--       İki yönde de çözülmüş gönderim şartını ve `pair_streak_max` üst
--       sınırını KENDİ İÇİNDE uyguluyor.
--   leave_pair_streak → tek dokunuşla çıkış. DSA Md. 28(1) Kılavuzu
--       küçükler için kolay çıkış istiyor; çıkış karşı tarafa BİLDİRİLMİYOR,
--       yoksa ayrılmak sosyal olarak cezalandırılırdı.
--
-- LİSTEYE BİLEREK GİRMEYEN: `pair_streak_rollover`. Yalnızca cron çağırıyor
-- ve o `postgres` olarak koşuyor; istemciye açılması kullanıcının kendi
-- serisini elle ilerletmesi demekti. Aşağıdaki revoke döngüsü onu kapatıyor;
-- bu İSTENEN davranış ve `supabase/tests/320_pair_streaks.sql` ayrıca iddia
-- ediyor.
--
-- Bu dosyanın geri kalanı 0085'in AYNISI. Kapılar kümülatif.
