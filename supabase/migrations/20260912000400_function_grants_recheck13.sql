-- 0082 — Fonksiyon yetkisi beyaz listesi (Task 12 · P4 kapısı)
--
-- 0080 ile aynı kapı, güncellenmiş listeyle.
--
-- LİSTEYE GİREN (Task 12 · P4):
--   send_question_to_friends → istemci soruyu artık TEK çağrıda gönderiyor.
--       Günlük tavan, arkadaş başına tavan ve "aynı soruyu 30 gün tekrar
--       gönderme" yasağı bu fonksiyonun içinde; arkadaşlık/engel/askı
--       kontrolü RLS politikasında KALIYOR (tek doğruluk kaynağı).
--       `bump_rate_limit` herkesten revoke olduğu için istemci sayacı
--       kendisi artıramıyor — sarmalayıcı tam bu yüzden var.
--
-- LİSTEYE BİLEREK GİRMEYEN: `config_bool` (0080'in gerekçesi) ve
-- `bump_rate_limit` (0040'tan beri kapalı; açık olsaydı istemci kendi
-- sayacını doldurup sonra da kendi sınırını atlayamaz ama BAŞKA kovaları
-- kirletebilirdi).
--
-- Bu dosyanın geri kalanı 0080'in AYNISI. Kapılar kümülatif.
