...
--
-- MUTASYON: `send_question_to_friends`in SAHIPLIK kosulunu sok
-- (`m.user_id = v_uid`). Diger bes kosul (moderasyon, tarama, aski, anonim,
-- arkadaslik/engel) YERINDE kaliyor — mutasyon tek korumayi bozmali.
--
-- BEKLENEN: 085'in "BASKASININ sorusu gonderilemiyor" ve "red sebebi
-- not_sendable" iddialari kirmizi.
--
-- NEDEN BU BIR KORUMA: fonksiyon SECURITY DEFINER, yani `question_sends` ve
-- `mistakes` uzerindeki RLS DEGERLENDIRILMIYOR. Depo bu gercegi iki yerde
-- yaziyor (0062 sanctions.sql:517, 0063 guardian_removal.sql:79-81) ve hicbir
-- yerde `force row level security` yok. Ilk yazim politikanin alti kosulunu
-- "tek dogruluk kaynagi" sayip govdede TEKRARLAMAMISTI; `p_mistake` keyfi bir
-- uuid oldugu icin kullanici BASKASININ (ya da moderasyonda isaretlenmis, ya
-- da taramasi bitmemis) bir hata satirini arkadaslarina gonderebiliyordu.
--
